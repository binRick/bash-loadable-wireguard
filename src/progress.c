...
static char *replace_str(char *strbuf, char *strold, char *strnew)              {
  char *strret, *p = NULL;
  char *posnews, *posold;
...
void progress_event_listener_free(progress_event_listener_t *listener)      {
  ...
  if (listener->data)
  {
    progress_data_free(listener->data);
  }
...
bool progress_change_value(progress_t *progress, int value, bool increment)      {
